...
EOF
        ...
        local java_home
        java_home=$("$tool_path" "${tool_opts[@]}") || return 1
...
if tk_is_login_shell; then
    ...
    #
    # * `ignoreboth`: don't save the current line that begins with space
    #   or is a duplicate of the previous line
    #
    # * `erasedups`: delete all previous lines matching the current line
    ...
    tk_cmd_exist chnode && chnode node-26
